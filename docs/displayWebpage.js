function displaySchedule(fixColorModeSlider = true) {
    ...
    if (moment.tz(timeZone).utcOffset() === moment.tz("America/Los_Angeles").utcOffset()) {
        document.getElementById("queryExample").href = "https://www.mtgoupdate.com/?timeZone=America/New_York";
    } else {
        document.getElementById("queryExample").href = "https://www.mtgoupdate.com/?timeZone=America/Los_Angeles";
    }
    
    document.getElementById("localTime").innerHTML = moment.unix(today.valueOf() / 1000).tz(timeZone).format("hh:mm:ss A") + ` (${timeZone})`;
    ...
    let fieldSets = generateFieldsetsForWeek(schedule, new Date(today), timeZone, eventFilter); // each day's textbox pre-padding
    let max = 0;
    for (let k = 0; k < 7; k++) {
...
function generateFieldsetsForWeek(schedule, today, timeZone, eventFilter) {
    ...
    let schedules = getSchedulesForWeek(schedule, new Date(today), timeZone, eventFilter, isDarkMode);
    for (let i = 0; i < 7; i++) {
...
function getSchedulesForWeek(schedule, today, timeZone, eventFilter, isDarkMode) {
    let startDate = new Date(today);
    startDate.setDate(startDate.getDate() - 1);
    let monthPadding = startDate.getMonth() < 9 ? "0" : "";
    let datePadding = startDate.getDate() < 10 ? "0" : "";
    let parseDate = `${startDate.getFullYear()}-${monthPadding}${startDate.getMonth() + 1}-${datePadding}${startDate.getDate()} 00:00`;
         ...
         for (let index = 0; index < events.length; index++) {
             ...
             if (event) {
                 if (event === "DST") {
                     ...
                     let eventString = `<font color= #COLOR>${getPrettyTime(hour, minute)} ${event}</font><br>`;
    ...
    for (let i = 1; i < ret.length; i++) {
        ...
        if (sched) {
            ret[i] = sched
                    .map(x => x[0].replace("#COLOR", x[1]))
                    .join("");
            let event = sched[0];
            if (event[0].includes("12am")) {
                ret[i - 1] += event[0]
                                .replace("12am", "Midnight")
                                .replace("#COLOR", getEventColor(event[0], isDarkMode, true, i === 8));
            }
        }
        today.setDate(today.getDate() + 1);
    }
